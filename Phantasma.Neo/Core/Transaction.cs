...
namespace Phantasma.Neo.Core
{
    ...
    public class Witness
    {
        ...
        public Phantasma.Core.Cryptography.Address ExtractAddress()
        {
            if (this.verificationScript.Length == 0)
            {
                return Phantasma.Core.Cryptography.Address.Null;
            }
            var bytes = new byte[34];
            bytes[0] = (byte)Phantasma.Core.Cryptography.AddressKind.User;
            Phantasma.Core.Utils.ByteArrayUtils.CopyBytes(this.verificationScript, 1, bytes, 1, 33);
            return Phantasma.Core.Cryptography.Address.FromBytes(bytes);
        }
    }
    ...
    public class Transaction 
    {
        ...
        public void Sign(NeoKeys key, IEnumerable<Witness> witnesses = null)
        {
            ...
            if (key != null)
            {
                ...
                var signature = Phantasma.Core.Cryptography.CryptoExtensions.SignECDsa(txdata, privkey, pubkey, ECDsaCurve.Secp256r1);
        ...
        public UInt256 Hash
        {
            get
            {
                if (_hash == null)
                {
                    ...
                    _hash = new UInt256(Phantasma.Core.Cryptography.CryptoExtensions.Hash256(rawTx));
                }
        ...
        public Phantasma.Core.Cryptography.Address ExtractInteropAddress()
        {
            if (attributes != null)
            {
                foreach (var attr in attributes)
                {
                    if (attr.Usage == TransactionAttributeUsage.Description)
                    {
                        ...
                        if (Phantasma.Core.Cryptography.Address.IsValidAddress(text))
                        {
                            return Phantasma.Core.Cryptography.Address.FromText(text);
                        }
                    }
                }
            }
