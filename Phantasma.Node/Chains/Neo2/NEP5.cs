...
namespace Phantasma.Spook.Chains
{
    ...
    public class NEP5
    {
        ...
        {
            return BalanceOf(address.AddressToScriptHash());
        }
        ...
        {
            return Transfer(from_key, to_address.AddressToScriptHash(), value, nonce, usedRpc);
        }
        ...
        public Transaction Transfer(NeoKeys from_key, byte[] to_address_hash, decimal value, byte[] nonce = null, Action<string> usedRpc = null)
        {
            ...
            var sender_address_hash = from_key.Address.AddressToScriptHash();
        ...
        {
            return Allowance(from_address.AddressToScriptHash(), to_address.AddressToScriptHash());
    ...
    public static class TokenSale
    {
        ...
        {
            var attachs = new List<TransactionOutput>();
            attachs.Add(new TransactionOutput() { AssetId = new UInt256(NeoAPI.GetAssetID(symbol)), ScriptHash = token.ScriptHash, Value = Fixed8.FromDecimal(amount) });
