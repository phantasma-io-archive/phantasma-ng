...
public class NexusSimulator
{
    ...
    public NexusSimulator(PhantasmaKeys[] owners, int seed, Nexus nexus, int protocolVersion)
    {
        ...
        var token = Nexus.GetTokenInfo(Nexus.StorageCollection.ContractsStorage, DomainSettings.FuelTokenSymbol);
        var localBalance = Nexus.RootChain.GetTokenBalance(Nexus.StorageCollection.AddressBalancesStorage, token, _currentValidator.Address);
    ...
    public void TransferOwnerAssetsToAddress(Address target)
    {
        ...
        foreach (var validator in _validators)
        {
            ...
            var balance = rootChain.GetTokenBalance(Nexus.StorageCollection.AddressBalancesStorage, DomainSettings.StakingTokenSymbol, validator.Address);
            if (balance > 0)
            {
                GenerateTransfer(validator, target, rootChain, DomainSettings.StakingTokenSymbol, balance);
            }
            
            var balanceKCAL = rootChain.GetTokenBalance(Nexus.StorageCollection.AddressBalancesStorage, DomainSettings.StakingTokenSymbol, validator.Address);
            if (balanceKCAL > 0)
            {
    ...
    private void InitGenesis(int protocolVersion)
    {
        ...
        var initialBalance = Nexus.RootChain.GetTokenBalance(Nexus.StorageCollection.AddressBalancesStorage, DomainSettings.StakingTokenSymbol, _currentValidator.Address);
    ...
    {
        var neoPlatform = NeoWallet.NeoPlatform;
        var neoKeys = InteropUtils.GenerateInteropKeys(_currentValidator, Nexus.GetGenesisHash(Nexus.StorageCollection.PlatformsStorage), neoPlatform);
        var neoText = NeoKeys.FromWIF(neoKeys.ToWIF()).Address;
        var neoAddress = NeoWallet.EncodeAddress(neoText);

        var ethPlatform = EthereumWallet.EthereumPlatform;
        var ethKeys = InteropUtils.GenerateInteropKeys(_currentValidator, Nexus.GetGenesisHash(Nexus.StorageCollection.PlatformsStorage), ethPlatform);
        var ethText = EthereumKey.FromWIF(ethKeys.ToWIF()).Address;
        var ethAddress = EthereumWallet.EncodeAddress(ethText);

        var bscPlatform = BSCWallet.BSCPlatform;
        var bscKeys = InteropUtils.GenerateInteropKeys(_currentValidator, Nexus.GetGenesisHash(Nexus.StorageCollection.PlatformsStorage), bscPlatform);
        var bscText = EthereumKey.FromWIF(bscKeys.ToWIF()).Address;
        var bscAddress = BSCWallet.EncodeAddress(bscText);

        Nexus.CreatePlatform(Nexus.StorageCollection.PlatformsStorage, neoText, neoAddress, neoPlatform, "GAS");
        Nexus.CreatePlatform(Nexus.StorageCollection.PlatformsStorage, ethText, ethAddress, ethPlatform, "ETH");
        Nexus.CreatePlatform(Nexus.StorageCollection.PlatformsStorage, bscText, bscAddress, bscPlatform, "BNB");
    ...
    public void BeginBlock(PhantasmaKeys validator)
    {
        ...
        {
            var currentName = Nexus.RootChain.GetNameFromAddress(Nexus.StorageCollection.AddressStorage, address, CurrentTime);
    ...
    public IEnumerable<Block> EndBlock()
    {
        ...
        var protocol = (uint)Nexus.GetGovernanceValue(Nexus.StorageCollection.ContractsStorage, Nexus.NexusProtocolVersionTag);
    ...
    {
        var version = Nexus.GetGovernanceValue(Nexus.StorageCollection.ContractsStorage, Nexus.NexusProtocolVersionTag);
        if (labels == null)
        {
    ...
    public ITransaction GenerateSideChainSend(PhantasmaKeys source, string tokenSymbol, IChain sourceChain, Address targetAddress, IChain targetChain, BigInteger amount, BigInteger fee)
    {
        ...
        Throw.If(!Nexus.TokenExists(Nexus.StorageCollection.MainStorage, tokenSymbol), "Token does not exist: "+ tokenSymbol);
                ...
                case 5:
                    {
                        ...
                        var token = Nexus.GetTokenInfo(Nexus.StorageCollection.ContractsStorage, tokenSymbol);
                        ...
                        if (balance > fee + AccountContract.RegistrationCost && !pendingNames.Contains(source.Address))
                        {
                            ...
                            var currentName = Nexus.RootChain.GetNameFromAddress(Nexus.StorageCollection.AddressStorage, source.Address, CurrentTime);
                            if (currentName == ValidationUtils.ANONYMOUS_NAME)
                            {
                                var lookup = Nexus.LookUpName(Nexus.StorageCollection.AddressStorage, randomName, CurrentTime);
                                if (lookup.IsNull)
                                {
                ...
                default:
                    {
                        ...
                        if (source.Address != targetAddress)
                        {
                            ...
                            var fuelToken = Nexus.GetTokenInfo(Nexus.StorageCollection.ContractsStorage, DomainSettings.FuelTokenSymbol);
    ...
    {
        var archive = Nexus.GetArchive(Nexus.StorageCollection.ArchiveStorage, hash);
        if (archive == null)
        {
    ...
    {
        return this.Nexus.RootChain.InvokeContractAtTimestamp(Nexus.StorageCollection.ContractsStorage, CurrentTime, nativeContract, methodName, args);
    }

    public VMObject InvokeContract(string contractName, string methodName, params object[] args)
    {
        return this.Nexus.RootChain.InvokeContractAtTimestamp(Nexus.StorageCollection.ContractsStorage, CurrentTime, contractName, methodName, args);
    }

    public VMObject InvokeScript(byte[] script)
    {
        return this.Nexus.RootChain.InvokeScript(Nexus.StorageCollection.ContractsStorage, script, CurrentTime);
    }
