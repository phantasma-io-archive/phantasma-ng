    ...
    public class AuctionController : BaseControllerV1
    {
        ...
        public int GetAuctionsCount(
            [APIParameter("Chain address or name where the market is located", "main")] string chainAddressOrName =
                null, [APIParameter("Token symbol used as filter", "NACHO")] string symbol = null)
        {
            ...
            IEnumerable<MarketAuction> entries = (MarketAuction[])chain
                .InvokeContractAtTimestamp(chain.Storage, Timestamp.Now, "market", "GetAuctions").ToObject();
        ...
        [HttpGet("GetAuctions")]
        public PaginatedResult GetAuctions(
            [APIParameter("Chain address or name where the market is located", "NACHO")] string chainAddressOrName,
            [APIParameter("Token symbol used as filter", "NACHO")] string symbol = null,
            [APIParameter("Index of page to return", "5")]
            uint page = 1,
            [APIParameter("Number of items to return per page", "5")]
            uint? pageSize = null)
        {
            var service = ServiceUtility.GetAPIService(HttpContext);
            pageSize = !pageSize.HasValue ? service.PaginationMaxResults : pageSize;
            ...
            pageSize = pageSize > service.PaginationMaxResults ? service.PaginationMaxResults : pageSize;
            ...
            paginatedResult.pageSize = pageSize.Value;
            paginatedResult.totalPages = totalPages;
            paginatedResult.total = numberRecords;
            paginatedResult.page = page;

            paginatedResult.result = entries.Select(x => service.FillAuction(x, chain)).ToArray();
        ...
        [HttpGet("GetAuction")]
        public AuctionResult GetAuction(
            [APIParameter("Chain address or name where the market is located", "NACHO")] string chainAddressOrName,
            [APIParameter("Token symbol", "NACHO")] string symbol, [APIParameter("Token ID", "1")] string IDtext)
        {
            var service = ServiceUtility.GetAPIService(HttpContext);
            ...
            try
            {
                var info = service.ReadNft(symbol, ID);
            }
            catch (Exception e)
            {
            ...
            var nft = service.ReadNft(symbol, ID);

            var forSale = chain
                .InvokeContractAtTimestamp(chain.Storage, Timestamp.Now, "market", "HasAuction", symbol, ID).AsBool();
            if (!forSale)
            {
                throw new APIException("Token not for sale");
            }

            var auction = (MarketAuction)chain
                .InvokeContractAtTimestamp(chain.Storage, Timestamp.Now, "market", "GetAuction", symbol, ID).ToObject();
